# 猿人学04-css干扰



## 网页分析

通过点击数据页,查看网络请求数据,可以发现

请求返回的是一串html 标签,并且图片是以base64格式存在的,

接着在响应体中,搜索img标签,发现数量远超实际显示的数量,说明并不是每一张图片都被显示.

而且响应体中有两个很特殊的值,分别叫key和value,

如此,就需要跟踪请求的栈,去查看对响应体的处理方式:

![image-20220315151735741](img/猿人学04_css干扰/image-20220315151735741.png)

代码很直白,没有分析难度,语法不会的直接百度,都有说明,

1.取出响应体中的那一段标签

2.找到 class=number 的标签,将其内容替换

3.使用响应体中的key和value生成一段字符串 j_key

4.找到class = j_key 的标签,将其设置为不可见

5.清除掉多余的属性名称

看清步骤之后,再去查看响应体中的img标签,发现果然class里都有两个属性,只要其中包含之后计算出来的 j_key,那就是无效元素

还有一点规律是img标签都相对left有一个位移,且位移值都是11.5的倍数,这一点只要查看元素顺序,很容易就能发现.

最后再使用js-RPC技术注入源JS,获取j_key 字符串,就可以完成数据爬取

## 准备Sekiro

RPC使用详情请查看: [Sekiro Hook Web JS 的详细使用步骤示例](./pyton/行者web爬虫.md)

Sekiro-clint 代码如下:

```js
<script>
    my_hook_xy = function (key, value) {
    	// 只需要 j_key 的接口即可
        return hex_md5(btoa(key + value).replace(/=/g, ''));
    };


    (function () {
        // 这里拷贝sekiro-clint上半部分代码

        // 这里是修改好的 sekiro-clint 下半部分代码
        function guid() {
            function S4() {
                return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1);
            }

            return (S4() + S4() + "-" + S4() + "-" + S4() + "-" + S4() + "-" + S4() + S4() + S4());
        }

        const client_sekiro = new SekiroClient("ws://127.0.0.1:5620/business-demo/register?group=yuan_ren_xue_04&clientId=" + guid());

        client_sekiro.registerAction("yuan_ren_xue_04", function (request, resolve, reject) {
            resolve(my_hook_xy(request['key'], request['value']));
        })
    })();
</script>
```



python爬虫代码如下:

```python
import re

import config
import requests

# 图片base64与数字值的映射字典
number_dict = {
    '': 1,
    '': 2,
    '': 3,
    '': 4,
    '': 5,
    '': 6,
    '': 7,
    '': 8,
    '': 9,
    '': 0
}


def get_number_list(page):
    url = f'{config.host}/api/match/4'
    params = {
        'page': page
    }
    res = requests.get(url=url, headers=config.headers, params=params).json()
    print(res)
    info = res['info']

    params = {
        'group': 'yuan_ren_xue_04',
        'action': 'yuan_ren_xue_04',
    }
    data = {
        'key': res['key'],
        'value': res['value']
    }
    result = requests.post('http://127.0.0.1:5620/business-demo/invoke', params=params, data=data).json()
    del_class_name = result['data']
    number_list = []
    for item in re.finditer(r'<td>.*?</td>', info, re.S):
        td = item.group()
        temp_list = []
        n = 0
        for item2 in re.finditer(
                r'<img src="data:image/png;base64,(?P<base64>.*?)" class="img_number (?P<class_name>.*?)" style="left:(?P<move>.*?)px">',
                td, re.S):
            base_64 = item2.group('base64')
            class_name = item2.group('class_name')
            move = int(float(item2.group('move')) / 11.5)
            if class_name != del_class_name:
                number = number_dict[base_64]
                temp_list.append((number, n + move))
                n += 1
        temp_list.sort(key=lambda x: x[1])
        temp_list = [str(item[0]) for item in temp_list]
        number_list.append(int(''.join(temp_list)))

    print(number_list)
    return number_list


if __name__ == '__main__':
    total_list = []
    for i in range(1,6):
        total_list.extend(get_number_list(i))

    print(sum(total_list))

```

启动sekiro,执行python程序

![image-20220315153911660](img/猿人学04_css干扰/image-20220315153911660.png)



