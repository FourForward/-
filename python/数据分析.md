...
**在jupyter notebook 中查看函数源码**

[Jupyter notebook 使用文档及查看函数源码 - Leon&Joker - 博客园 (cnblogs.com)](https://www.cnblogs.com/leon-joker/p/12659460.html)

```python
示例
pd.Series.str.contains??	# 查看对象源码
pd.Series.str.contains?		# 查看对象文档
dir(pd.Series.str.contains)	# 查看对象的所有属性和方法(只查名称)
```
...
[API文档: API reference — pandas 1.4.1 documentation (pydata.org)](https://pandas.pydata.org/pandas-docs/stable/reference/index.html)
...
[Series.str源码: pandas/accessor.py at 1.4.x · pandas-dev/pandas (github.com)](https://github.com/pandas-dev/pandas/blob/1.4.x/pandas/core/strings/accessor.py#L143)

**正则表达式匹配 Series.str.extract**

[源码: pandas/accessor.py at 1.4.x · pandas-dev/pandas (github.com)](https://github.com/pandas-dev/pandas/blob/1.4.x/pandas/core/strings/accessor.py#L2439-L2441)

```python
# 正则表达式匹配 Series.str.extract（self，pat，flags=0，expand=True）
products['prod_name'].str.extract('(?P<begin>Jet).*?(?P<prod_name>1000)')

products['prod_name'].str.extract('(1000|2000)', expand=False)
```

| 参数   | 说明                                                         |
| ------ | ------------------------------------------------------------ |
| pat    | 正则表达式,`必须包含捕获组`,每个捕获组为返回值的一列         |
| flags  | int，默认为0（无标志）来自re模块的标志<br />例如re.IGNORECASE，修改正则表达式匹配，例如大小写，空格等。<br />有关详细信息，请参阅re:https://docs.python.org/3/library/re.html#module-re |
| expand | bool，默认为True   只有一个捕获组的情况下应该取False,否则会报错<br/>如果为True，则返回DataFrame，每个捕获组一列。<br />如果为False，如果有一个捕获组，则返回Series/索引;<br />                          如果有多个捕获组，则返回DataFrame。 |
| 返回值 | DataFrame/Series/索引<br/>一个DataFrame，每个主题字符串有一行，每个组有一列。<br />正则表达式pat中的任何捕获组名称都将用于列名称; 否则将使用捕获组编号。<br />即使找不到匹配项，每个结果列的dtype也始终为object。<br />如果 expand=False且pat只有一个捕获组，则返回一个Series/索引。<br/> |

**字符串拼接 Series.str.cat**
...
```python
Series.str.lower : 将所有字符转换为小写
Series.str.upper : 将所有字符转换为大写
Series.str.title : 将每个单词的第一个字符转换为大写，其余的转换为小写
Series.str.capitalize: 将第一个字符转换为大写，其余字符转换为小写
Series.str.swapcase: 将大写转换为小写，小写转换为大写
Series.str.casefold: 删除字符串中的所有大小写差异。
...
# 也可以新增多列
df[['e','f']] = [[1,2],[1,2],[1,2],...]	# 有多少行就需要多少个元素

```

使用Series的str属性,添加多列(适用于简单的计算)

```py
import pandas as pd

df = pd.DataFrame(data=[[10001,'越前辣椒油160ml'],[10004,'金师傅酱油1750ml'],[10007,'金龙鱼调和油800ml']],columns=['商品编码','商品名称'])
# 需求将商品名称中的名称和规格,拆分成新的两列

df[['new_name','weight']] = df['商品名称'].str.extract(r'(?P<name>.*?)(?P<weight>\d+ml)')

###
  商品编码 商品名称          new_name    weight
0 10001   越前辣椒油160ml   越前辣椒油    160ml
1 10004   金师傅酱油1750ml  金师傅酱油    1750ml
2 10007   金龙鱼调和油800ml  金龙鱼调和油  800ml
###
```



使用apply一次添加多列(适用于复杂计算)
...
def split_name(row):
    old_name = row['商品名称']
    result = re.search(r'(?P<name>.*?)(?P<weight>\d+ml)',old_name)
    return result.group('name'), result.group('weight')

df[['new_name','weight']] = df.apply(split_name,axis=1, result_type='expand')
...
```python
# 改列名 更改列名
df.rename(columns={'旧列名1':'新列名1', '旧列名2':'新列名2'})	# 返回新的dataframe
```
...
```py
df.loc[(df['Height']<0.00244), 'Height_type'] = 'A'		# 在原数据上修改
```
...
df = pd.read_excel('路径',sheet='工作薄名称')
```
...
参数详解:[pandas数据合并之一文弄懂pd.concat（） - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/132593960)
...
| `as_index` | 接收boolearn,默认为True,表示是否将聚合列作为新的索引         |
| sort       | 接收boolearn。表示是否对分组依据分组标签进行排序。默认为True。 |
| dropna     | 布尔值, 默认为True, 表示忽略NAN值所在的行                    |
...
# 对于那些 np 没有提供的聚合函数,在使用时,可以直接传入字符串,比如:
grouped.agg({'age':'max', 'score':'mean'})
```
...
对于想要在`聚合后保留其余的列`,可以在分组时将这些要保留的列加入条件中去,以达到保留这些列的目的.(前提:在当前分组条件下,这些列的值是完全相同的,或者说,将这些列作为分组条件,不会对分组结果产生影响)

```python
# 数据初始化
df = pd.merge(left=customers, right=orders, on=['cust_id'], how='inner')[['cust_id', 'order_num', 'cust_name']]
# 以 cust_id 分组,但是希望在聚合后的结果中保留 cust_name 字段,因为cust_id 和 cust_name 1:1对应,所以可以将 cust_name 直接加入分组条件
df.groupby(['cust_id', 'cust_name'], as_index=False).agg({'order_num': 'count'})
```

对于想要在聚合后,将聚合列的改名(改列名)的方法:

```python
# 正式改列名方法
df.groupby(['x'], as_index=False).agg({'order_num': 'count'}).rename(columns={'order_num':'count'})

# 新增子列名的方法
df.groupby(['x'], as_index=False).agg({'order_num': ['count']})

# 使用 count() 聚合时,经常返回 Series 数据类型, 此时可以使用: `reset_index(name='给原来的列取个新名字,一般取count')`,如果聚合函数返回的时 dataframe类型数据,则不能使用 name 参数.
```
...
![](我的笔记园/小玉的玉/python/数据分析/images/箱线图.png)
...
```python
# 如果只想填充某一列的缺失值，需要先将该列单独取出来，进行填补后在合并回去
df['columns'] = df[['columns']].fillna(method='ffill')['columns']
```
...
![](我的笔记园/小玉的玉/python/数据分析/images/股票差函数多项式拟合.png)
